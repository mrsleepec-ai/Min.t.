...
(function(){
  let lastHtml = null;
  const btnShare = document.getElementById('pdfShareBtn');
...
btnShare.onclick = async (e)=>{
  e.preventDefault();
  try{
    // Use the same HTML we show in the viewer
    const html = (function(){ try { return window.__lastPdfHtml || null; } catch(_) { return null; } })();
    let blob;
    if(html){
      blob = new Blob([html], {type:'text/html'});
    } else {
      // Try to read from iframe if available
      const frame = document.getElementById('pdfFrame');
      if(frame && frame.src && frame.src.startsWith('blob:')){
        // open the existing blob directly
        window.open(frame.src, '_blank');
        return;
      } else {
        alert('Откройте документ в Safari через кнопку рядом.');
        const btnSafari = document.getElementById('pdfOpenSafari');
        if(btnSafari) btnSafari.click();
        return;
      }
    }
    const url = URL.createObjectURL(blob);
    window.open(url, '_blank'); // always open in Safari -> system Share sheet available
    setTimeout(()=>URL.revokeObjectURL(url), 15000);
  }catch(err){
    alert('Не удалось открыть в Safari. Используйте кнопку «Открыть в Safari».');
    const btnSafari = document.getElementById('pdfOpenSafari');
    if(btnSafari) btnSafari.click();
  }
};
