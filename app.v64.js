console.log('MINT_VERSION','v64',1757243260);
// Mint
...
function foRender(){
  ...
  arr.forEach(it=>{
    fo.list.appendChild(itemRow(t,it));
  });
}
// events
try{
  ...
  fo.add.onclick = ()=>{
    ...
    fo.input.value=''; save(); foRender(); renderChecklist && renderChecklist(t);
  };
  fo.del.onclick = ()=>{
    const t=current.task, f=current.folder; if(!t||!f) return;
