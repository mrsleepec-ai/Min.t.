...
// === Folders feature (move to folder + open folder view) – injected ===
function __ff_ensureModals(){
...
function __ff_showPicker(taskId,itemId){
  __ff_ensureModals();
  const t=tasks.find(x=>x.id===taskId); if(!t) return;
  const it=(t.items||[]).find(i=>i.id===itemId); if(!it) return;
...
function __ff_openFolder(taskId,folderId){
  __ff_ensureModals();
  const t=tasks.find(x=>x.id===taskId); if(!t) return;
